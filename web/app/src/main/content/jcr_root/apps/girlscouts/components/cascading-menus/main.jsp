...
<%
 // Boolean isInHamburger = false;
  Boolean isInHamburger =(Boolean)request.getAttribute("isInHamburger");
  ...
  if(isInHamburger){
	  menuBuilder.append("<ul class=\"off-canvas-list\">");
  ...
  if(eventGrandParent.equalsIgnoreCase(currentSite.get("eventPath", String.class))){
      ...
      if(isInHamburger){
    	  ...
    	  menuBuilder.append("<li><label><a href=\"").append(eventResource.getPath()+".html\"/>").append(eventResource.adaptTo(Page.class).getTitle()).append("</a></label></li>");
      }
  ...
  else if(currPath.equalsIgnoreCase(currentPage.getAbsoluteParent(2).getPath()))
  {
     ...
     navigationPath = new HashSet<String>(Arrays.asList(navigation.split("/")));
      ...
      if(isInHamburger){
    	  Resource parentLabel = resourceResolver.getResource(currentPage.getAbsoluteParent(3).getPath());
          menuBuilder.append("<li><label><a href=\"").append(parentLabel.getPath()+".html\"/>").append(parentLabel.adaptTo(Page.class).getTitle()).append("</a></label></li>");
  ...
  int levelDepth = 0;
  buildMenu(menuLevel1,navigationPath,menuBuilder,levelDepth,flag, insertAfter, currPath, currTitle, isInHamburger);
...
<%=menuBuilder %>
<%!
  public StringBuilder buildMenu(Iterator<Page>menuLevel1, Set<String> navigationPath, StringBuilder menuBuilder, int levelDepth, boolean flag, String dispEvtBelow, String currPath, String currTitle, boolean isInHamburger){
    levelDepth++;
    ...
    if(menuLevel1.hasNext())
    {
     ...
     while(menuLevel1.hasNext())
     {
          ...
          if(navigationPath.contains(level1.getName()) && !level1.isHideInNav() && level1.listChildren().hasNext() && levelDepth < 3){
              buildMenu(level1.listChildren(),navigationPath,menuBuilder,levelDepth,flag,dispEvtBelow,currPath,currTitle,isInHamburger);
          }
          menuBuilder.append("</li>");
          if (levelDepth == 1 && !isInHamburger) {
