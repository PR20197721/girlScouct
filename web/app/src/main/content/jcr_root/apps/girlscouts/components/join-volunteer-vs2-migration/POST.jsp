                ...
                try {
                    ...
                    while(contentChildren.hasNext()){
                        ...
                        if(site.hasProperty("jcr:primaryType") && site.getProperty("jcr:primaryType").getString().equals("cq:Page") ){
                            ...
                            sql = "SELECT * FROM [nt:unstructured] as s where ISDESCENDANTNODE(["+site.getPath()+"]) AND CONTAINS(s.*, '"+joinVolunteerMigration.getOldVolunteer()+"')";
                            log.debug("executing "+sql);
                            q = qm.createQuery(sql, "JCR-SQL2");
                            result = q.execute();
                            rowIter = result.getRows();
                            while (rowIter.hasNext()) {
                                if(this.stop){
                                    log.debug("Stopping JoinVolunteerMigration");
                                    return;
