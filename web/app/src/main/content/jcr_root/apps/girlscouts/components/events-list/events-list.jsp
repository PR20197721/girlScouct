...
<cq:defineObjects />
<%
    SearchResultsInfo srchInfo = (SearchResultsInfo) request
		    .getAttribute("eventresults");
    if (null == srchInfo) {
%>
	<cq:include script="/apps/girlscouts/components/event-search/event-search.jsp" />
<%
    srchInfo = (SearchResultsInfo) request
			.getAttribute("eventresults");
...
<div>
	<img src='<%=properties.get("fileReference", String.class)%>'
		width="50" height="50" alt="" />
	<h2>
		<a href='<%=properties.get("urltolink")%>.html'><%=properties.get("featuretitle", String.class)%></a>
	</h2>
</div>
...
<%
    for (int i = 0; i < eventcounts; i++) {
		...
		Node node = resourceResolver.getResource(value).adaptTo(
			Node.class);
