...
(function (window, document, Granite, $) {
    ...
    function submit(wizard) {
        ...
        var successMessage = "";
        var errorMessage = "";
        var $dest = $("input[name='dest']");
        $dest.val($dest.val() + "/" + $("#rename").val());
        var moveTagSettings = $("#movetagsettings");
        ...
        successMessage = Granite.I18n.get("Tag Moved Successfully");
        errorMessage = Granite.I18n.get("Failed to move tag");
        $.ajax({
            ...
            if ($("#tagmove-success").length === 0) {
                var insertModal = $("<div>", {"class": "coral-Modal", "id": "tagmove-success", "style": "width:30rem"}).hide().html(MODAL_HTML);
                $(document.body).append(insertModal);
        	}
            var successModal = new CUI.Modal({
                element: '#tagmove-success',
                ...
                buttons: [{
                        ...
                        click: function (evt) {
                            this.hide();
                            location.href = Granite.HTTP.externalize(moveTagSettings.data("redirect"));
                        }
                    }
                ]
            });
        	...
        	if ($("#tagmove-failure").length === 0) {
                var insertModal = $("<div>", {"class": "coral-Modal", "id": "tagmove-failure", "style": "width:30rem"}).hide().html(MODAL_HTML);
                $(document.body).append(insertModal);
        	}
            var failureModal = new CUI.Modal({
                element: '#tagmove-failure',
