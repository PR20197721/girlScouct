...
var abacus = jQuery.abacus = (function ($) {
    ...
    function getFunction(definition, functionName) {
        ...
        if (functionDef instanceof Array && functionDef.length === 2) {
            ...
            // Wrap leftOperand of showHideExpression with context object in order to allow for hyphens in reference
            var func = functionDef.toString(),
                regex = new RegExp(/(?:\(\()(.*?)(?:\.equals\(\")/, "g"), // https://regex101.com/r/L8JbDr/7
                matches = {},
                m;

            while ((m = regex.exec(func)) !== null) {
                matches[m[1]] = true; // [1] is first and only regex capture group, Prevent duplicate matches by adding string to object
            }
            for (m in matches) {
                func = func.split(m).join("$c['" + m + "']"); // Replace all instances of match with wrapped string
            }

            // $c is the context - parent Entity containing all Properties
    ...
    function EntityBase() {
        ...
        this.$value = function (newJSON) {
            var p, propertyType, newProp, definition = this.$$private.definition,
                parent = this.$parent();
    ...
    function CollectionBase() {
        ...
        this.$remove = function (nIndex) {
            var deletedRow = this.$item(nIndex),
                i, len;
    ...
    $m = {
                        ...
                        if (sType === "available" || this.currentProperty.$isAvailable()) {
                            ...
                            try {
                                ...
                                if (phase === RETRY_PHASE) {
                                    alert("This is the error:" + err.toString());
                                } else {
        ...
        eventListener: function (observer, srcObj, eventName, fn) {
            ...
            this.synchronize = function () {
                var currentValue = srcObj.valueOf(),
                    thisEvent;
