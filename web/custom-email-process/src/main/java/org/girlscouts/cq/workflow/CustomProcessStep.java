        ...
        if (workflowData.getPayloadType().equals(TYPE_JCR_PATH)) {
            ...
            try {
                ...
                if (node != null) {
                   ...
                   //If template text is not found try to get template from path
                   if(emailTemplate == null || emailTemplate.equals("")){
                	   templatePath =  args.get("templatePath", String.class);
                       
                       try{
    	                   Node content = jcrSession.getNode(templatePath + "/" + "jcr:content");
                   ...
                   //If emailTemplate is still not found or is empty log an error
                   if(emailTemplate == null || emailTemplate.equals("")){
                   ...
                   String firstLine = emailTemplate.substring(0, emailTemplate.indexOf('\n'));
                   String unrefinedMessage = emailTemplate.substring(emailTemplate.indexOf("\n") + 1);
                   String to = firstLine.substring(0, firstLine.indexOf(':'));
                   String address = firstLine.substring(firstLine.indexOf(':') + 1);
