...
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=Girl Scouts Web to Case Servlet",
		...
		"sling.servlet.selectors=webtocase", "sling.servlet.resourceTypes=foundation/components/form/start" })
public class WebToCaseServlet extends SlingAllMethodsServlet implements OptingServlet {
	...
	protected void doPost(SlingHttpServletRequest request, SlingHttpServletResponse response) throws IOException {
        ...
        if (errors != null && errors.size() > 0) {
            ...
            if (isSendEmail) {
                webToCase.sendEmail(request, data, errors, debug);
            }else {
			...
			for (RequestParameter paraValue : paras) {
				if (paraValue.isFormField()) {// do not support file upload
					if (expectedParams.contains(paraName)) {
						...
						// additional fields that are not expected by salesforce will be appended to
						// description
	...
	private void sendConfirmationEmail(SlingHttpServletRequest request, ValueMap props) {
		try {
			...
			final String confSubject = formProps.get("confirmationSubject",
					resBundle.getString("Form Submission Received"));
			logger.debug("confSubject = {}", confSubject);
			String templatePath = formProps.get("templatePath",
			...
			if (!StringUtils.isBlank(confBody)) {
				...
				if(!confirmationEmailAddresses.isEmpty()) {
