...
curl -u admin:admin -F action=install -F bundlestartlevel=20 -F bundlefile=@"./target/girlscouts-vtk-replication-bundle-1.0-SNAPSHOT.jar" http://localhost:4506/system/console/bundles
#!/bin/bash
...
for server in ${SERVER_LIST[@]}; do
        ...
        for port in ${PORT_LIST[@]}; do
                ...
                if [ $? -ne 0 ]; then
                        ...
                        curl -u admin:admin -F action=install -F bundlestartlevel=20 -F bundlefile=@"./target/girlscouts-vtk-replication-bundle-1.0-SNAPSHOT.jar" http://$server:$port/system/console/bundles
                fi
        done
done
