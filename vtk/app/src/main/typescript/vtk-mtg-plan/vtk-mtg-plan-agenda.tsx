...
import {HAS_PERMISSION_FOR, IS_MEETING_CANCEL, PERMISSION_CHECK} from './permission';
        ...
        <div>
            <ul className={(items.disabledText) ? "__agenda-items __past-year" : "__agenda-items"}>
                {items.collection.map((value, index) => {
                    return (<SortableItem key={`item-${value.uid}-${index}`} index={index} value={{
                        value,
                        api,
                        remove,
...
export function VtkMtgPlanAgenda(props: VtkMtgPlanAgendaProps) {
    ...
    return (<div>
        <VtkContent idName="agenda">
            ...
            {(PERMISSION_CHECK()('permission_edit_meeting_id') && !IS_MEETING_CANCEL()) ?
                <a className="add-btn" data-reveal-id="modal_agenda">
