...
import {DATE_MORE_THAN_1977, HAS_PERMISSION_FOR, IS_MEETING, IS_MEETING_CANCEL, PERMISSION_CHECK} from './permission';
    ...
    if (helper.currentDate && DATE_MORE_THAN_1977(helper.currentDate) && !IS_MEETING_CANCEL()) {
        return <p>
            ...
            {(meetingEvents.sentEmails && meetingEvents.sentEmails.length > 0)
                ? <span className="a-tag-modal"><br/>{`( ${meetingEvents.sentEmails.length} sent - `}<Link
                    modal={'modal_view_sent_emails'} title={`view`}/>{` )`}</span>
                : null
            }
        </p>
    ...
    return (
        <div>
            <VtkContent idName="comunications">
                {(permission || user_variable.user_current_year !== user_variable.vtk_current_year) ?
                    <div><h6>Manage Communications</h6>
