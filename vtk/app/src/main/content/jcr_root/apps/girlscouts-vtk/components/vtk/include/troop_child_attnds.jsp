    ...
    <div class="column large-20 large-centered">
        <div class="row">
            <dl class="accordion-inner clearfix" data-accordion>
                ...
                %><p>Meetings attended by <%=contact.getFirstName() %> <%= contacts.size()%>
            </p><%
                for (int y = 0; y < infos.size(); y++) {
                    if (infos.get(y).isAttended()) {
            %>
                <ul class="inline-list">
                    <li><%=VtkUtil.formatDate(VtkUtil.FORMAT_Md, (java.util.Date) sched_bm_inverse.get(infos.get(y).getYearPlanComponent()))%>
                    </li>
                </ul>
                <%
                            }
