...
<ul>
    <%
        ...
        for (int i = 0; i < activities.size(); i++) {
            ...
            if (selectedTroop.getYearPlan().getActivities() != null) {
                for (int y = 0; y < selectedTroop.getYearPlan().getActivities().size(); y++) {
                    if (selectedTroop.getYearPlan().getActivities().get(y).getName().equals(activities.get(i).getName())) {
                        isExists = true;
                        break;
                    }
                }
    ...
    <li class="searchResultsItem">
        <h4 class="activityName"><%=activities.get(i).getName()%>
        </h4>
        <p class="activityDate">
            <strong>Date: <%=activities.get(i).getDate() == null ? "" : VtkUtil.formatDate(VtkUtil.FORMAT_MEETING_REMINDER, activities.get(i).getDate()) %>
                -
                <%=activities.get(i).getEndDate() == null ? "" : VtkUtil.formatDate(VtkUtil.FORMAT_MEETING_REMINDER, activities.get(i).getEndDate()) %>
            </strong></p>
        <p class="activityLocation"><strong>Location:
            ...
            <a href="javascript:void(0)" class="reserved"
               onclick="showMap('<%=activities.get(i).getLocationAddress().replace("\r","")%>')">Map</a>
            <% } %>
        </strong></p>
        <%if (activities.get(i).getContent() != null) { %>
        <p class="activityContent"><%=activities.get(i).getContent().trim() %>
        </p>
        <% } %>
        ...
        <%} else { %>
        <i class="activityDisabled" id="act_lib_msg_added_<%=activities.get(i).getUid()%>" style="display:none;">This
            Activity has already been selected</i>
