      ...
      return <li className="row"  key={item.activityNumber} id={item.activityNumber}>
        <div className="wrapper clearfix">
          <div className="large-3 medium-3 small-3 columns small-push-1 large-push-2">
            <span>{  moment('<%=planView.getSearchDate()%>').format('YYYY') <1978 ? item.activityNumber : moment( getAgendaTime( item.duration )).format("HH:mm")} </span>
          </div>
            ...
            <div className="large-3 medium-3 small-3 columns">
              <span>:{item.duration<9 ? ("0"+item.duration) : item.duration}</span>
            </div>
          </div>
        </li>;
  				}))	
