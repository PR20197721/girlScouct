...
<%
    ...
    if( selectedTroop!=null && selectedTroop.getYearPlan()!=null){ 
    	meetingEvents = selectedTroop.getYearPlan().getMeetingEvents();
    	}
    csv.append("Legend:  X = attendance; A = achievement\n");
    // doc title
    csv.append(FORMAT_MMM_dd_yyyy.format( new Date() ) +" "+
            (selectedTroop.getSfTroopAge().substring(selectedTroop.getSfTroopAge().indexOf("-") + 1)) + "  "
			+ selectedTroop.getSfTroopName());
	...
	if (contacts != null)
		for (Contact gsContact : contacts) {
			...
			if (gsContact.getDob() != null) {
				try {
					dob = VtkUtil.formatDate(VtkUtil.FORMAT_MMddYYYY,
							VtkUtil.parseDate(VtkUtil.FORMAT_yyyyMMdd, gsContact.getDob()));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
			...
			csv.append(fmtValue((caregiver == null ? ""
					: (caregiver.getFirstName() == null ? "" : caregiver.getFirstName())) + " "
					+ ((caregiver.getLastName() == null ? "" : caregiver.getLastName()))) + ",");
			csv.append(fmtValue(gsContact.getEmail()) + ","
					+ (gsContact.getPhone() == null ? "" : fmtValue(gsContact.getPhone())) + "," + fmtValue(dob)
					+ "," + fmtValue(age) + ","
			...
			if (gsContact.getContacts() != null) {
				for (Contact contactSub : gsContact.getContacts()) {
					if (!VtkUtil.hasPermission(selectedTroop,
							Permission.PERMISSION_CAN_VIEW_OWN_CHILD_DETAIL_TROOP_ID)) {
						csv.append(fmtValue(contactSub.getFirstName() + " " + contactSub.getLastName()) + ",");
						if (VtkUtil.hasPermission(selectedTroop,
								Permission.PERMISSION_SEND_EMAIL_ALL_TROOP_PARENTS_ID)) {
			...
			if (meetingEvents != null) {
				...
				for (MeetingE meetingEvent : meetingEvents) {
					...
					for (ContactExtras info : infos) {
						...
						//acvm
						if (info.isAchievement()
								&& info.getYearPlanComponent().getType() == YearPlanComponentType.MEETING) {
							isAchievement = true;
						}
					}
