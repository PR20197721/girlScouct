...
<%@ page import="java.util.Map, java.text.SimpleDateFormat,java.util.Calendar, org.girlscouts.vtk.auth.permission.Permission" %>
...
<%
    ...
    if(meetingEvents!=null){
        for( MeetingE meetingEvent: meetingEvents ){
            if(meetingEvent.getDate() != null){
				calendar.setTime(meetingEvent.getDate());
				meetingDate = FORMAT_MMM_dd_yyyy.format(calendar.getTime()).toString()+",";

            }else{
				meetingDate = "Date,";
            }

			csv.append(meetingDate);
            if(meetingEvent.getMeetingInfo()==null ){
                Meeting meetingInfo = yearPlanUtil.getMeeting(user, selectedTroop, meetingEvent.getRefId());
                header = header + fmtValue(meetingInfo.getName()) +",";
            }else{
                header = header + fmtValue( meetingEvent.getMeetingInfo().getName()) +",";
            }
        }
    }
