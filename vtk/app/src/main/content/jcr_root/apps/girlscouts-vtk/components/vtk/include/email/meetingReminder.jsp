...
%>
	...
	<%= FORMAT_MEETING_REMINDER.format(searchDate) %> - <%=VtkUtil.formatDate(VtkUtil.FORMAT_hhmm_AMPM, meetingEndDate)%></h4>
...
<!-- <li>
	...
	<section class="clearfix">
		...
		<input type="text" id="email_subj" value="Reminder <%=troop.getTroop().getGradeLevel() %> Meeting #<%=planView.getMeetingCount()%> <%= FORMAT_MEETING_REMINDER.format(searchDate) %> - <%= VtkUtil.formatDate(VtkUtil.FORMAT_hhmm_AMPM,meetingEndDate)%>" />	
	</section>
	...
	<textarea id="email_htm" name="textarea" class="jqte-test" rows="25" cols="25">
		...
		<table> 
			<tr><th>Date:</th>
				<td><%= FORMAT_MEETING_REMINDER.format(searchDate)%> - <%= VtkUtil.formatDate(VtkUtil.FORMAT_hhmm_AMPM, meetingEndDate)%></td>
			</tr>
			<tr><th>Location:</th>
				<td><%
