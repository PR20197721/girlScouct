                        ...
                        if (hasVisibleChildren(page)) {
                            ...
                            while (grandChildren.hasNext()) {
                                ...
                                if (!p.isHideInNav()) {
                                    ...
                                    menuBuilder.append("<a href="+generateLink(currentPage, slingRequest, resourceResolver, p.getPath())+">" + p.getTitle() + "</a>");
                                    menuBuilder.append("</li>");
                                }
                            }
...
<%
    ...
    if (currentNode.hasNode("links")) {
        ...
        while (iter.hasNext()) {
            ...
            if (linkNode.hasProperty("large") && linkNode.hasProperty("url") && linkNode.hasProperty("medium") && linkNode.hasProperty("small")) {
...
<ul class="<%=flyoutClass%> inline-list">
    <%
        ...
        for (int i = 0; i < links.length; i++) {
            ...
            String activeStatus = "";
            if (!currPath.equals(rootPath) && ((currPath.startsWith(eventPath) && eventLeftNavRoot.startsWith(menuPath)) || menuPath.indexOf(currPath) == 0 || currPath.startsWith(menuPath))) {
                activeStatus = "active";
            }
            if (path.toLowerCase().contains("vtk")) {
    %>
    <li>
        <a data-dropdown="drop1" aria-controls="drop1" class="<%= clazz %> show-for-large-up menuHighlight" aria-expanded="false"><%= label %>
        </a>
        <a data-dropdown="drop1" aria-controls="drop1" class="<%= clazz %> show-for-medium-only menuHighlight" aria-expanded="false"><%= mLabel %>
        </a>
    ...
    <li data-link="<%=path%>" class="<%=hasChildren%><%=activeStatus%>">
        <a class="show-for-large-up menu <%=clazz%>" href="<%= generateLink(currentPage, slingRequest, resourceResolver,path)%>"><%=label%>
        </a>
        <a class="show-for-medium-only menu <%=clazz%>" href="<%= generateLink(currentPage, slingRequest, resourceResolver,path)%>"><%=mLabel%>
        </a><%
        try {
            if (flyPage != null) {
                String flyRight = (i <= links.length / 2) ? " right" : "";
                flyoutMap.put(path, buildFlyOutMenu(currentPage, slingRequest, resourceResolver,flyPage, flyRight));
