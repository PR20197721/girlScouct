        ...
        for (int i = 0; i < links.length; i++) {
            ...
            <%
                ...
                if (clazz.indexOf("hide-in-nav") < 0) { // If not hidden, create list element
                    String mappedPath = "http://" + resourceResolver.map(path);
                    %><li data-link="<%=path%>" class="<%=hasChildren%><%=activeStatus%>">
                        <a class="show-for-large-up menu <%=clazz%>" href="<%=mappedPath%>"><%=label%></a>
                        <a class="show-for-medium-only menu <%=clazz%>" href="<%=mappedPath%>"><%=mLabel%></a><%
                        try {
                            if (flyPage != null) {
