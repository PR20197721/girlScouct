...
<p>Configure <%=request.getParameter("mCountUpd") %> meeting dates starting on or
    after <%=VtkUtil.formatDate(VtkUtil.FORMAT_MMddYYYY, new java.util.Date(Long.parseLong(startAlterDate))) %>:</p>
<%} %>
<input type="hidden" id="orgDt" name="orgDt"
       value="<%=( startAlterDate!=null && !startAlterDate.trim().equals("")) ? startAlterDate:( selectedTroop.getYearPlan().getCalStartDate()==null ? "" : new java.util.Date(selectedTroop.getYearPlan().getCalStartDate()).getTime() ) %>"/>
<section class="clearfix">
    <div class="columns small-24 medium-8">
        <div class="row">
            <div class="small-15 medium-19 columns date">
                <input type="text" placeholder="Start Date" id="calStartDt" name="calStartDt"
                       value="<%=( startAlterDate!=null && !startAlterDate.trim().equals("")) ? VtkUtil.formatDate(VtkUtil.FORMAT_MMddYYYY, new java.util.Date( Long.parseLong(startAlterDate))):( selectedTroop.getYearPlan().getCalStartDate()==null ? "" : VtkUtil.formatDate(VtkUtil.FORMAT_MMddYYYY, new java.util.Date(selectedTroop.getYearPlan().getCalStartDate()))) %>"/>
            </div>
    ...
    <div class="columns small-24 medium-16">
        <div class="row">
            ...
            <div class="small-10   medium-7 columns left">
                <select id="calFreq">
                    <option value="weekly" <%= selectedTroop.getYearPlan().getCalFreq().equals("weekly") ? " SELECTED" : "" %>>
                        weekly
                    </option>
                    <option value="biweekly" <%= selectedTroop.getYearPlan().getCalFreq().equals("biweekly") ? " SELECTED" : "" %>>
                        biweekly
                    </option>
                    <option value="monthly" <%= selectedTroop.getYearPlan().getCalFreq().equals("monthly") ? " SELECTED" : "" %>>
                        monthly
                    </option>
                </select>
            </div>
...
<section class="clearfix holidays">
    ...
    <ul class="small-block-grid-2 medium-block-grid-3">
        ...
        %>
        <li>
            <input type="checkbox" id="chk_<%=holidayCount %>" name="exclDt"
                   value="<%=holidayDateFmt %>" <%=("".equals(exlDates) || exlDates.contains(holidayDateFmt)) ? "CHECKED" : ""  %>/>
            <label style="display:block;" for="chk_<%=holidayCount%>">
                <div style="padding-left: 30px;">
                    <span style="margin-left: 5px;"
                          class="date"><%=holidayDateFmt %></span><span><%=holidayTitle %></span>
                </div>
            </label>
        </li>
        <%
        ...
        %>
        <li>
            <input type="checkbox" id="chk_<%=(holidayCount) %>" name="exclDt" value="<%=split_exclDates[i] %>"
                   CHECKED/>
            <label style="display:block;" for="chk_<%=holidayCount%>">
                <div style="padding-left: 30px">
                    <span style="margin-left: 5px;"
                          class="date"><%= split_exclDates[i]%><br><%=holidayTitle == null ? "Canceled Meeting" : holidayTitle %></span>
                </div>
            </label>
        </li>
        <%
        ...
        <li>
            <input type="checkbox" id="chk_<%=holidayCount %>" name="exclDt"
                   value="<%=holidayDateFmt %>" <%=("".equals(exlDates) || exlDates.contains(holidayDateFmt)) ? "CHECKED" : ""  %>/><label
                for="chk_<%=holidayCount%>"><p><span
                class="date"><%=holidayDateFmt %></span><span><%=holidayTitle %></span></p></label>
        </li>
        <%
                            }//end if
                        } catch (Exception e) {
