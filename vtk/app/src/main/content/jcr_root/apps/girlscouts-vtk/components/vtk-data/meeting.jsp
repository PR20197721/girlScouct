...
<%
    ...
    if(currentNode.hasNode("activities")){
        NodeIterator iter = currentNode.getNode("activities").getNodes();
        while (iter.hasNext()) {
            Node node = iter.nextNode();
            Long activityNum = new Long(0);
            if(node.hasProperty("activityNumber")){
                try {
                    Property activityNumber = node.getProperty("activityNumber");
                    if (activityNumber.getType() == PropertyType.STRING) {
                        activityNum = Long.parseLong(node.getProperty("activityNumber").getString());
                    } else {
                        activityNum = node.getProperty("activityNumber").getLong();
                    }
                }catch (Exception e){

                }
            }
            SortItem activity = new SortItem(activityNum, node);
