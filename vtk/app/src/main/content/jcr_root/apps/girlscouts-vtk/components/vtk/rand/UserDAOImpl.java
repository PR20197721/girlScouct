...
public class UserDAOImpl implements UserDAO {
	...
	public Troop getUser(String userId) {
		...
		try {
			...
			// GOOD user = (User)
			// ocm.getObject("/content/girlscouts-vtk/users/"+ userId);
			 ...
			 * (user.getYearPlan().getLastAssetUpdate().before( new
			 * java.util.Date() ) && user.getYearPlan().getAssets()!=null &&
			 * user.getYearPlan().getAssets().size()>0 ) ){
			 * 
			 * java.util.List<Asset> assetsToRm = new java.util.ArrayList();
			 * 
			 * 
			 * java.util.Iterator itr = user.get if(
			 * user.getYearPlan().getAssets().get(i).iisCachable() )
	...
	public boolean updateUser(Troop user) {
		...
		try {
			...
			if (user == null || user.getYearPlan() == null) {
				System.err.println("exiting updateUser");
				return true;
			}
			...
			// - Depricated: info pulled to display from scafolding
			// user.getYearPlan().setMilestones(
			// meetingDAO.getCouncilMilestones( user.getTroop().getCouncilId() )
			// );
			...
			if (session.itemExists(user.getPath())) {
				...
				StringTokenizer t = new StringTokenizer(
						("/" + user.getPath()).replace("/" + user.getId(), ""),
						"/");
				int i = 0;
				...
				// ocm.insert( new JcrNode( user.getPath().replace(
				// "/"+user.getId(), "") ) );
	...
	public void selectYearPlan(Troop user, String yearPlanPath, String planName) {
		...
		try {
			...
			/*
			 * if( oldPlan==null || oldPlan.getName()==null ||
			 * oldPlan.getName().equals("") ){ YearPlan plan = new
			 * YearPlanDAOImpl().getYearPlan(yearPlanPath); if( plan!=null)
		...
		if (user.getYearPlan().getActivities() != null) {
			...
			for (int i = 0; i < user.getYearPlan().getActivities().size(); i++)
				if (new java.util.Date().before(user.getYearPlan()
	 ...
	 * 
	 * 
	 * YearPlan oldPlan = user.getYearPlan(); YearPlan newYearPlan =
	 * addYearPlan(user, yearPlanPath);
	 * 
	 * if( oldPlan==null || oldPlan.getMeetingEvents()==null ||
	 * oldPlan.getMeetingEvents().size()<=0 || oldPlan.getSchedule()==null ||
	 * oldPlan.getSchedule().getDates().equals("")){ //new user new plan, first
	 * time user.setYearPlan(newYearPlan); updateUser(user); return; }
	 * 
	 * 
	 * 
	 * 
	 ...
	 * 
	 * int maxMeetingCount = oldMeetings.size(); if( newMeetings.size()>
	 * maxMeetingCount) maxMeetingCount= newMeetings.size();
	 * 
	 * int countLockedMeetings = doCountLockedMeetins(
	 * oldPlan.getSchedule().getDates() ); //first n past meetings
	 * 
	 * for(int i=countLockedMeetings;i<maxMeetingCount;i++){
	 * 
	 * if( oldMeetings.size()>i && newMeetins.size()>i ){ //replace meeting @i
	 * copyMeeting( oldMeetings, newMeetings, i ); setCancelled("false");
	 * 
	 * }else if( oldMeetings.size() > newMeetings.size() ){ //remove meeting @i
	 * - downsize removeMeeting( oldMeetings, i); removeDates(i);
	 * 
	 * }else if( oldMeetings.size() < newMeetings.size() ){ //add new meeting @i
	 * insertMeeting( newMeetings, oldMeetings, i); insertDates(i); }
	 * 
	 * 
	 * }
	 * 
	 * user.getYearPlan().setActivities(null); updateUser(user);
	 * 
	 * }
	 */
	...
	public void addAsset(Troop user, String meetingUid, Asset asset) {
		...
		java.util.List<MeetingE> meetings = user.getYearPlan()
				.getMeetingEvents();
		 ...
		 * user.getYearPlan().getActivities(); for(int
		 * i=0;i<activities.size();i++) if( activities.get(i).getUid().equals(
		 * meetingUid)) activities.get(i).getAssets().add( asset );
		 */
	...
	public void loadUserGlobConfig() {
		...
		try {
			...
			userGlobConfig = (UserGlobConfig) ocm
	...
	/*
	 * public boolean hasPermission(Set<Integer> myPermissionTokens, int
	 * permissionId){ if( myPermissionTokens!=null &&
	 * myPermissionTokens.contains(permissionId) ) return true;
	 * 
	 * return false; }
	 */
}// ednclass
