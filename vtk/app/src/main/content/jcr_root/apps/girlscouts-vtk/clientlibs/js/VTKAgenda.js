                            ...
                            "enable": {
                                ...
                                "listeners": {
                                    "check": function (component) {
                                        var dlg = component.findParentByType('rtelinkdialog');
                                        dlg.enableSCFields(component.checked);
                                    }
                                }
                            },
                            "events": {
...
//if the NASA checked box is checked, it will return an array of length 1 with true in it. If unchecked, it will return an array with length 0..
        ...
        if (this.isMainAgenda) {
            this.isMulitpleAgendaLabel = new CQ.Ext.form.Label({
                text: "Multiple Agenda?",
        ...
        this.durationField = new CQ.Ext.form.NumberField({
            ...
            listeners: {
                change: {
                    scope: this,
                    fn: function (me, val) {
                        if (this.durationConst.indexOf(val) == -1) {
                            alert("Invalid duration: must be one of the following: " + this.durationConst);
                            me.setValue(5);
                        }
...
//        this.add(this.descriptionFieldDelete);
        ...
        this.materialFieldLabel = new CQ.Ext.form.Label({
            text: "Material List",
            disabled: ((this.isMainAgenda && !this.isNasaChecked) ? true : false)
        });
