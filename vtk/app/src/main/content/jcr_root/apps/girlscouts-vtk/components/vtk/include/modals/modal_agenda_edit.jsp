    ...
    <div class="scroll content" style="">
        ...
        <div class="setupCalendar row">
            <%
                ...
                List<MeetingE> meetings = selectedTroop.getYearPlan().getMeetingEvents();
                ...
                if (meeting == null) {
                    List<MeetingCanceled> cmeetings = selectedTroop.getYearPlan().getMeetingCanceled();
                ...
                Meeting meetingInfo = yearPlanUtil.getMeeting(user, selectedTroop, meeting.getRefId());
                List<Activity> _activities = meetingInfo.getActivities();
            ...
            <div class="row" style="background-color: white;">
                <form onsubmit="return false;">
                    <dl class="tabs agenda-tabs">
                        ...
                        %>
                        <dd id="tab_<%= sa %>" data-uid="<%= _subActivity.getUid() %>"><i class="dot"></i><a
                                href="#panel_<%= sa %>"><%=_subActivity.getName() %>
                        </a></dd>
                        <%
                                }
                        ...
                        %>
                        <dd id="tab_0" data-uid="<%= _subActivity_selected.getUid() %>"><i class="dot on"></i><a
                                href="#panel_0"><%=_subActivity_selected.getName() %>
                        </a></dd>
                        <%
                            }
                        %>
                    ...
                    <div class="columns small-6">
                        ...
                        <select id="_vtk_select_multiple_agenda"
                            ...
                            <option value="10"
                                    <%=(_subActivity_selected.getDuration() == 10) ? "SELECTED" : ""%>>10
                            </option>
                            <option value="15"
                                    <%=(_subActivity_selected.getDuration() == 15) ? "SELECTED" : ""%>>15
                            </option>
                            <option value="20"
                                    <%=(_subActivity_selected.getDuration() == 20) ? "SELECTED" : ""%>>20
                            </option>
                            <option value="25"
                                    <%=(_subActivity_selected.getDuration() == 25) ? "SELECTED" : ""%>>25
                            </option>
                            <option value="30"
                                    <%=(_subActivity_selected.getDuration() == 30) ? "SELECTED" : ""%>>30
                            </option>
                            <%} else { %>
                            <option value=""><%=_subActivity_selected.getDuration() %>
                            </option>
                            <%} %>
                        </select>
                        <%
                    ...
                    <div class="columns small-18">
                        ...
                        <button class="btn button"
                                onclick="return rmAgenda('<%=_subActivity_selected.getPath()%>', '<%=meeting.getPath()%>')">
            ...
            <section class="row agenda-panelwrap tabs-content" style="background-color: white; margin:0px -5px;">
                    ...
                    <div class="row">
                        ...
                        <div class="columns small-18">
                            <div class="select_multiple_agenda" style="margin-top:20px"><input
                                    ...
                                    id="<%= _subActivity.getPath().substring(_subActivity.getPath().lastIndexOf("/")+1).replace(".","")  %>"
                                    class="act-select" name="act-select" data-uid="<%= _subActivity.getUid() %>"
                                    type="radio" data-meeting='<%=meeting.getPath()%>'
                                    data-activity='<%=_activity.getPath()%>'
                                    data-subactivity='<%=_subActivity_selected.getPath()%>'
                                    onclick="onChangeHandler(event)"
                                <%= (_subActivity.getIsSelected()) ? "checked" : "" %>> <label
                                    for="<%= _subActivity.getPath().substring(_subActivity.getPath().lastIndexOf("/")+1).replace(".","")  %>"><span></span>
                                <p>Select this activity</p></div>
                        </div>
                    </div>
                    <div class="row"><%=_subActivity.getActivityDescription() %>
                    </div>
                </div>
                <%
                    }
                ...
                <div class="content" id="panel_0">
                    <div><%=_activity.getActivityDescription() %>
                    </div>
                </div>
                <%
                    }
