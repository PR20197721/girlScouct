<%@ page
        import="org.girlscouts.vtk.auth.permission.Permission,org.girlscouts.vtk.models.Asset" %>
    ...
    <div class="content">
        <table width="90%" align="center" class="browseMeetingAids">
            ...
            <tr>
                ...
                <td><a class="previewItem LINK" title="<%= a.getTitle() %>" href="<%=a.getRefId() %>"
                ...
                <td>
                    <% if (!existingAids.contains(a.getRefId()) && VtkUtil.hasPermission(selectedTroop, Permission.PERMISSION_EDIT_MEETING_ID)) { %>
                    <input type="button"
                           style="    min-width: 100px;     padding: 9px 5px;white-space: normal; word-break: break-word;"
                           value="Add to Meeting"
                           onclick="assignAid('<%=a.getRefId()%>', '<%=planView.getYearPlanComponent().getUid()%>', '<%=a.getTitle()%>','<%=a.getDocType()%>')"
