    ...
    <div class="scroll">
        <div class="content">
            <div class="browseSentEmails"><%
                ...
                <div>Sent: <%=VtkUtil.formatDate(VtkUtil.FORMAT_CALENDAR_DATE, eml.getSentDate()) %>
                </div>
                <div><%=curA.getEmlTemplate() == null ? "" : eml.getHtmlMsg(curA.getEmlTemplate())%>
                </div>
                <%
                } else {%>
                <div class="scroll_1">
                    <ul id="email-list"><%
                        ...
                        <li class="<%= k==emails.size()? "active" : ""%>"><a href="#nogo"
                                                                             data-target="panel<%=k%>s"><%=eml.getSubject()%>
                        </a><span
                                class="right"><%=VtkUtil.formatDate(VtkUtil.FORMAT_MONTH_DAY, eml.getSentDate()) %></span>
                        </li>
                        <% } %>
                    </ul>
                </div>
                <div class="scroll_2"><%
                    ...
                    <div id="panel<%=k%>s" class="<%= k==emails.size()? "show" : "hide"%>">
                        ...
                        <div>Sent: <%=VtkUtil.formatDate(VtkUtil.FORMAT_MMMM_dd_hhmm_AMPM, eml.getSentDate())%>
                        </div>
                        <div><%=curA.getEmlTemplate() == null ? "" : eml.getHtmlMsg(curA.getEmlTemplate())%>
                        </div>
                    </div>
                ...
                <div><%=curM.getEmlTemplate() == null ? "" : eml.getHtmlMsg(curM.getEmlTemplate())%>
                </div>
                <%
                } else {%>
                <div class="scroll_1">
