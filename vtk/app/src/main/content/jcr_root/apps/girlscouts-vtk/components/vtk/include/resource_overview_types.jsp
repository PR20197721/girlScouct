<div class="__overview-types row">
    ...
    <div class="__list-radio columns small-24 medium-19 end">
        ...
        <div class="small-24 medium-12 large-8 columns">
            <input type="radio" name="meetingTypes" id="<%=meetingPlayType%>" value="<%=meetingPlayType%>"
                   onclick="showMeetingTypes('<%=meetingPlayType%>')"/>
            <label for="<%=meetingPlayType%>"><span></span>
                <p><%=meetingPlayType.replace("_", " ")%> (<%=sortMeetingByMeetingType.get(meetingPlayType).size()%>
                    ) </p></label>
        </div>
        <%}%>
        <div class="small-24 medium-12 large-8 columns">
            ...
            <label for="all"><span></span>
                <p> All (<%=raw_meetings.size()%>) </p></label>
            <%}%>
        </div>
    </div>
</div>
