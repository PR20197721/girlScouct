...
<table class="list-of-meeting-calendar combine-meeting yearMeetingList">
    <%
        for (int i = 0; i < meetingsToCancel.size(); i++) {
            ...
            while (ii.hasNext()) {
                ...
                if (dt.before(new java.util.Date()) || sched.get(dt) instanceof Activity) {
                    continue;
                }
    ...
    <tr>
        <td>
            ...
            <label for="y<%=meetingsToCancel.get(i).getUid() %>"><span></span>
                <p></p></label>
        </td>
        <td><%=VtkUtil.formatDate(VtkUtil.FORMAT_CALENDAR_DATE, dt)%>
        </td>
