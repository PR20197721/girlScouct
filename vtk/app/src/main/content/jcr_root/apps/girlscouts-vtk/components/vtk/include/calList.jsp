...
<p>Select the <i class="icon-calendar"></i> to change the date, time, or cancel an individual meeting.
    ...
    Select the <i class="icon-gear"></i> to use the planning wizard to reconfigure the calendar from that date forward.
</p>
<table cellpadding="5" cellspacing="0" class="yearMeetingList" width="100%">
    ...
    <tr>
        ...
        <td><span><%= _meeting.getName() %>
				...
				<% } %></span></td>
        <td class="vtk_age_level <%= _meeting.getLevel() %>"><%= (_meeting.getLevel() == null && _meeting.getLevel().length() > 0) ? "" : _meeting.getLevel().charAt(0) %>
        </td>
        <td>
            <% if (date != null && date.after(new java.util.Date())) {%>
            <a onclick="showAlterYearPlanStartDate('<%= date.getTime() %>', '<%= (sched.size() +1 - currentMeeting) %>')"
               title="settings"><i class="icon-gear"></i></a>
            <%} %>
        </td>
    </tr>
    <% }%>
