...
public class CalendarUtil {
    ...
    public boolean updateSched(User user, Troop troop, String meetingPath, String time, String date, String ap, String isCancelledMeeting, long currDate) throws java.lang.IllegalAccessException, VtkException {
        ...
        if ((sched == null || sched.contains(newDate.getTime() + "")) && !(("" + currDate).equals(newDate.getTime() + ""))) {
    ...
    public void resetCal(User user, Troop troop) throws java.lang.IllegalAccessException, VtkException {
    ...
    public String getSchedDates(User user, Troop troop, String freq, org.joda.time.DateTime newStartDate, String exclDate, long oldFromDate) throws java.lang.IllegalAccessException {
        if (troop != null && !userUtil.hasPermission(troop, Permission.PERMISSION_EDIT_MEETING_ID)) {
            throw new IllegalAccessException();
        }
        // org Dates
    ...
    public YearPlan getSched(User user, Troop troop, String freq, org.joda.time.DateTime newStartDate, String exclDate, long oldFromDate) throws java.lang.IllegalAccessException {
        String dates = getSchedDates(user, troop, freq, newStartDate, exclDate, oldFromDate);
        YearPlan plan = troop.getYearPlan();
    ...
    public void createSched(User user, Troop troop, String freq, org.joda.time.DateTime newStartDate, String exclDate, long oldFromDate) throws java.lang.IllegalAccessException, VtkException {
        if (troop != null && !userUtil.hasPermission(troop, Permission.PERMISSION_EDIT_MEETING_ID)) {
            throw new IllegalAccessException();
        }
    ...
    public long getNextDate(List<String> exclDates, long theDate, String freq, boolean isUseCurrDate) {
        ...
        if (nextDate <= 0 || (nextDate > new java.util.Date().getTime()) && !exclDates.contains(fmtDate.format(new java.util.Date(nextDate)))) {
            return nextDate;
        } else {
    ...
    private long getMonthlyNextDate(org.joda.time.DateTime date) {
        ...
        while (cal.get(java.util.Calendar.MONTH) == month) {
            ...
            if (count == weekOfMonth && dayOfWeek == cal.get(java.util.Calendar.DAY_OF_WEEK)) {
    ...
    public long getMaxYearPlanDate(long startYearPlanDate) {
        ...
        sepThisYear.set(java.util.Calendar.DAY_OF_MONTH, VtkUtil.getCurrentGSDate());
        sepThisYear.set(java.util.Calendar.MONTH, VtkUtil.getCurrentGSMonth() - 1);
        ...
        if ((!VtkUtil.isSameDate(sepThisYear.getTime(), _startYearPlanDate.getTime())) && sepThisYear.getTimeInMillis() > _startYearPlanDate.getTimeInMillis()) {
            return sepThisYear.getTimeInMillis();
        } else {
            java.util.Calendar now = java.util.Calendar.getInstance();
            int maxYear = now.get(java.util.Calendar.YEAR);
            if ((now.get(java.util.Calendar.MONTH) > VtkUtil.getCurrentGSMonth() - 1) || ((now.get(java.util.Calendar.MONTH) == VtkUtil.getCurrentGSMonth() - 1) && (now.get(java.util.Calendar.DATE) >= VtkUtil.getCurrentGSDate()))) {
                maxYear += 1;
            }
    ...
    public boolean isEventPastGSYear(User user, Troop troop) {
        if (troop.getYearPlan() == null || troop.getYearPlan().getSchedule() == null || troop.getYearPlan().getSchedule().getDates() == null || "".equals(troop.getYearPlan().getSchedule().getDates())) {
            return true;
        }
    ...
    public boolean updateDate(User user, Troop troop, long currDate, long newDate) throws java.lang.IllegalAccessException, VtkException {
        if (troop != null && !userUtil.hasPermission(troop, Permission.PERMISSION_EDIT_MEETING_ID)) {
            throw new IllegalAccessException();
        }
        java.text.SimpleDateFormat dateFormat4 = new java.text.SimpleDateFormat("MM/dd/yyyy hh:mm a");
    ...
    public void combineMeeting(User user, Troop troop, String currDates, String _dt) throws java.lang.IllegalAccessException, java.text.ParseException, VtkException {
