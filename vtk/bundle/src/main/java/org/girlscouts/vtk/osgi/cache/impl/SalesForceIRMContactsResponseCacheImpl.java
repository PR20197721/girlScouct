...
import org.girlscouts.vtk.osgi.conf.SalesForceIRMContactsResponseCacheConfig;
...
@Component(service = {SalesForceIRMContactsResponseCache.class}, immediate = true, name = "org.girlscouts.vtk.osgi.cache.impl.SalesForceIRMContactsResponseCacheImpl")
@Designate(ocd = SalesForceIRMContactsResponseCacheConfig.class)
public class SalesForceIRMContactsResponseCacheImpl extends BasicGirlScoutsService implements SalesForceIRMContactsResponseCache{

    private static Logger log = LoggerFactory.getLogger(SalesForceIRMContactsResponseCacheImpl.class);
    ...
    private void activate(ComponentContext context) {
        ...
        log.info("Girl Scouts VTK IRM contacts cache Activated.");
    }
    ...
    public boolean contains(String key) {
        if (this.isCacheEnabled && key != null) {
            ...
            try {
                ...
                log.error("Exception reading from cache irm contacts for " + key + " troop", e);
            }
        }
        return false;
    }
    ...
    public void write(String key, ContactsInfoResponseEntity entity) {
        if(this.isCacheEnabled && key != null && entity != null && entity.getContacts() != null && entity.getContacts().length > 0) {
            try {
                ...
                log.error("Exception occurred adding object to irm cache for key=" + key, e);
