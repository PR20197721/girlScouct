...
public class DateImporterTimestamperImpl implements DataImportTimestamper {
    ...
    private static final Logger log = LoggerFactory.getLogger(DateImporterTimestamperImpl.class);
    ...
    private void loadFromJcr() {
        try {
            timestamp = getSession().getProperty(TIMESTAMP_PATH + "/" + TIMESTAMP_PROP).getDate().getTime();
    ...
    private void saveToJcr(Date date) {
        try {
            ...
            cal.setTime(date);
            JcrUtil.createPath(TIMESTAMP_PATH, "nt:unstructured", getSession());
            getSession().getNode(TIMESTAMP_PATH).setProperty(TIMESTAMP_PROP, cal);
            session.save();
        } catch (RepositoryException e) {
    ...
    private Session getSession() throws RepositoryException {
        if (session == null) {
            if (repository == null) {
                log.error("Cannot get reference to Sling repository");
