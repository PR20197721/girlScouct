...
public class TroopUtil {
	...
	public Troop getTroop(User user, String councilId, String troopId)
		 ...
		 * troop.getYearPlan().getActivities(); if (activities != null) for (int
		 * i = 0; i < activities.size(); i++) {
		 * 
		 * 
		 * if ((activities.get(i).getCancelled() == null || activities
		 * .get(i).getCancelled().equals("false")) &&
		 * !activities.get(i).getIsEditable() && activities.get(i).getRefUid()
		 * != null) {
		 * 
		 * 
		 * Activity a = activityDAO.findActivity(user, activities.get(i)
		 * .getRefUid()); if (a == null) activities.get(i).setCancelled("true");
		 * else{ // TRANSFER DATA ATTR FROM A TO .get(i) //activities.set(i, a);
		 * }
		 * 
		 * } }
		 */
