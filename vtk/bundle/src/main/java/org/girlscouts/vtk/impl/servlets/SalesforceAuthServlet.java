...
public class SalesforceAuthServlet extends SlingSafeMethodsServlet implements
	...
	public boolean logoutApi(ApiConfig apiConfig, boolean isRefreshToken)
		...
		try {
			...
			if (isRefreshToken) {
				log.debug("Revoke refresh token " + apiConfig.getRefreshToken());
				urlParameters = "token=" + apiConfig.getRefreshToken();
			} else
				log.debug("Revoke token " + apiConfig.getAccessToken());
