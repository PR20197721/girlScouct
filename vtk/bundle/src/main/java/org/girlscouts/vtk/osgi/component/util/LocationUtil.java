...
public class LocationUtil {
    ...
    public void setLocationAllMeetings(User user, Troop troop, String locationPath) throws java.lang.IllegalAccessException, VtkException {
    ...
    public void setLocation(User user, Troop troop, Location location) throws java.lang.IllegalAccessException, VtkException {
        if (troop != null && !userUtil.hasPermission(troop, Permission.PERMISSION_EDIT_MEETING_ID)) {
            throw new IllegalAccessException();
        }
        YearPlan plan = troop.getYearPlan();
    ...
    public void changeLocation(User user, Troop troop, String dates, String locationRef) throws java.lang.IllegalAccessException, VtkException {
        if (troop != null && !userUtil.hasPermission(troop, Permission.PERMISSION_EDIT_MEETING_ID)) {
            throw new IllegalAccessException();
        }
        ...
        for (int i = 0; i < meetings.size(); i++) {
            if (meetings.get(i).getLocationRef() != null && meetings.get(i).getLocationRef().equals(locationRef) && !processedMeetings.contains(meetings.get(i).getPath())) {
    ...
    public void setLocationAllEmpty(User user, Troop troop, String locationName) throws java.lang.IllegalAccessException, VtkException {
    ...
    public void removeLocation(User user, Troop troop, String locationName) {
        try {
            String locationToRmPath = meetingDAO.removeLocation(user, troop, locationName);
            YearPlan plan = troop.getYearPlan();
            // update every refLoc & set 2 null
            java.util.List<MeetingE> meetings = plan.getMeetingEvents();
            for (int i = 0; i < meetings.size(); i++) {
                if (meetings.get(i).getLocationRef() != null && meetings.get(i).getLocationRef().equals(locationToRmPath)) {
                    meetings.get(i).setLocationRef("");
                    meetings.get(i).setDbUpdate(true);
                }
            }
    ...
    public void addLocation(User user, Troop troop, Location location) throws IllegalAccessException, VtkException {
        boolean isLoc = VtkUtil.isLocation(troop.getYearPlan().getLocations(), location.getName());
        if (!isLoc) {
