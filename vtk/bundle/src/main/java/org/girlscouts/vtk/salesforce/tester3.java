...
// ***  Send bulk Email to salesForice
		...
		email.setSubject("test email"+ new java.util.Date());
...
// *** end  Send bulk Email to salesForice		
	...
	public void createAccount(Account _account, ApiConfig apiConfig){
		...
		post.setRequestHeader("Authorization", "OAuth " + apiConfig.getAccessToken());
			...
			if (get.getStatusCode() == HttpStatus.SC_OK) {
				...
				try {
					...
					for (int i = 0; i < results.length(); i++) {
						...
						acc.setName(results.getJSONObject(i).getString("Name"));
						accounts.add(acc);
	...
	public void updateAccount(Account _account, ApiConfig apiConfig){
		...
		PostMethod patch = new PostMethod(apiConfig.getInstanceUrl()
				+ "/services/data/v20.0/sobjects/Account/" + _account.getId()) {
			@Override
...
public Contact showContacts(ApiConfig apiConfig, String campaignMemberId) {
	...
	params[0] = new NameValuePair("q",
			"SELECT Name, Id,Email from Contact where id='"+ campaignMemberId+"'");
	get.setQueryString(params);

	try {
		httpclient.executeMethod(get);
		
		System.err.println("Contact: "+ get.getStatusCode() +" :" + get.getResponseBodyAsString());
		...
		if (get.getStatusCode() == HttpStatus.SC_OK) {
			...
			try {
				...
				for (int i = 0; i < results.length(); i++) {
					...
					contact.setLastName(results.getJSONObject(i).getString("Name"));					
...
public void updateContact(Contact _contact, ApiConfig apiConfig){
	...
	PostMethod patch = new PostMethod(apiConfig.getInstanceUrl()
			+ "/services/data/v28.0/sobjects/Contact/" + _contact.getId()) {
		@Override
		public String getName() {
			return "PATCH";
		}
...
public String sendEmail(ApiConfig apiConfig, Email emailInfo){
	...
	try {
	/* THIS WORKS
		//email.put("ActivityId" , "asdf");
		email.put("FromAddress" , "alex_yakobovich@northps.com");
		email.put("HtmlBody" , "html body");
		email.put("TextBody" , "text test body");
		email.put("Subject" , "test"+ new java.util.Date());
		email.put("ToAddress" , "alex_yakobovich@northps.com");
		email.put("ParentId", "500Z0000007Szyg"); //good case"500Z0000007RPTRIA4");
		email.put("BccAddress", "test@test.com, test1@test.com, test2@test.com");
		*/
...
public String doEmail(ApiConfig apiConfig, Email email){
	...
	//compaign contactIds
	java.util.List <String> campaignMembers = getCampaignContactIds(apiConfig, campaign);
	
	//all contactIds
	java.util.Map allContacts =new tester3().getContactEmailList(apiConfig);
...
public java.util.List<String> getCampaignIds(ApiConfig apiConfig, String contactId){
	...
	System.err.println("UserId: "+ apiConfig.getUserId());
	    GetMethod get =null;
	    //java.util.List <CampaignMember> campaignMembers = new java.util.ArrayList();
...
System.err.println( "REsults: "+results.toString());
					for (int i = 0; i < results.length(); i++) {
						...
						campaignIds.add(results.getJSONObject(i).getString("CampaignId") );
						/*
						CampaignMember campaignMember= new CampaignMember();
						campaignMember.setId(results.getJSONObject(i).getString("CampaignId"));
						campaignMembers.add( campaignMember );
						*/
				//System.err.println("*** "+ apiConfig.getUserId() +" : "+ results.getJSONObject(i).getString("ContactId") );		
					/*	
	System.err.println( 
			...
			*/
	/*
	getCampaign(apiConfig, results.getJSONObject(i).getString("CampaignId"));
...
public java.util.List<String> getCampaignContactIds(ApiConfig apiConfig, Campaign campaign){
		...
		params[0] = new NameValuePair("q",
				"SELECT contactId from campaignmember where campaignId=\'"+campaign.getId()+"\'");
		get.setQueryString(params);
			...
			if (get.getStatusCode() == HttpStatus.SC_OK) {
				...
				try {
					...
					for (int i = 0; i < results.length(); i++) {
						...
						compaignMembers.add( results.getJSONObject(i).getString("ContactId"));
					}
...
public void logout(ApiConfig apiConfig)  {
  ...
  public java.util.List<Contact> getGirstEmails( ApiConfig apiConfig, String campaignId, String contactType ){

	  java.util.List<Contact> contacts = new java.util.ArrayList();
	  
	 java.util.List <String>campaignMemberIds =  getCampaignMemberIds(apiConfig, campaignId, contactType);
...
public java.util.List<String> getCampaignMemberIds(ApiConfig apiConfig, String campaignId, String contactType){
		...
		if( contactType!=null && !contactType.trim().equals("")){
		    params[0] = new NameValuePair("q",
				...
				" campaignMember where campaignId='"+campaignId+"' and Assignment_Type__c='"+ contactType +"'");
		}else{
			params[0] = new NameValuePair("q",
					"SELECT   ContactId from " +
...
System.err.println( "REsults: "+results.toString());
					for (int i = 0; i < results.length(); i++) {
						...
						campaignIds.add(results.getJSONObject(i).getString("ContactId") );
...
/*
{"attributes":{"type":"Campaign","url":"/services/data/v20.0/sobjects/Campaign/701G0000000tONiIAM"},"Id":"701G0000000tONiIAM","IsDeleted":false,"Name":"Unsure","ParentId":"701G0000000tONdIAM","Type":null,"RecordTypeId":"012G0000001IVBrIAO","Status":"Planned","StartDate":null,"EndDate":null,"ExpectedRevenue":null,"BudgetedCost":null,"ActualCost":null,"ExpectedResponse":0.0,"NumberSent":7.0,"IsActive":true,"Description":null,"NumberOfLeads":0,"NumberOfConvertedLeads":0,"NumberOfContacts":7,"NumberOfResponses":0,"NumberOfOpportunities":0,"NumberOfWonOpportunities":0,"AmountAllOpportunities":0.0,"AmountWonOpportunities":0.0,"OwnerId":"005G0000003tifCIAQ","CreatedDate":"2014-03-13T18:02:55.000+0000","CreatedById":"005G0000003tifCIAQ","LastModifiedDate":"2014-06-03T16:45:05.000+0000","LastModifiedById":"005Z0000002546lIAA","SystemModstamp":"2014-06-03T16:45:05.000+0000","LastActivityDate":null,"CampaignMemberRecordTypeId":"012G0000001IWnsIAG","rC_Giving__Affiliation__c":null,"rC_Giving__Appeal_Segment__c":null,"rC_Giving__Archive_Flag__c":false,"rC_Giving__Average_Gift_Hierarchy__c":null,"rC_Giving__Average_Gift__c":null,"rC_Giving__Best_Case_Amount_Hierarchy__c":0.0,"rC_Giving__Best_Case_Amount__c":0.0,"rC_Giving__Best_Case_Bar__c":"<img src=\"/img/samples/color_yellow.gif\" alt=\"yellow\" height=\"10\" width=\"0\" border=\"0\"/> 0%","rC_Giving__Best_Case_Ratio__c":0.0,"rC_Giving__Campaign_Category__c":null,"rC_Giving__Campaign_Type__c":null,"rC_Giving__Channel__c":null,"rC_Giving__Closed_Amount_Hierarchy__c":0.0,"rC_Giving__Closed_Amount__c":0.0,"rC_Giving__Closed_Bar__c":"<img src=\"/img/samples/color_green.gif\" alt=\"green\" height=\"10\" width=\"0\" border=\"0\"/> 0%","rC_Giving__Closed_Ratio__c":0.0,"rC_Giving__Commit_Amount_Hierarchy__c":0.0,"rC_Giving__Commit_Amount__c":0.0,"rC_Giving__Commit_Bar__c":"<img src=\"/img/samples/color_green.gif\" alt=\"green\" height=\"10\" width=\"0\" border=\"0\"/> 0%","rC_Giving__Commit_Ratio__c":0.0,"rC_Giving__Cost_Per_Gift__c":null,"rC_Giving__Cost_Per_Piece_Hierarchy__c":0.0,"rC_Giving__Cost_Per_Piece__c":0.0,"rC_Giving__Cost_Per_Thousand_Hierarchy__c":0.0,"rC_Giving__Cost_Per_Thousand__c":0.0,"rC_Giving__Cost_Per_Won_Opportunity__c":0.0,"rC_Giving__Creative_Package__c":null,"rC_Giving__Current_Giving_Amount_Hierarchy__c":0.0,"rC_Giving__Current_Giving_Amount__c":0.0,"rC_Giving__Days_Since_Drop__c":null,"rC_Giving__Drop_Date__c":null,"rC_Giving__Effort__c":null,"rC_Giving__End_Date_Time__c":null,"rC_Giving__Expected_Giving_Amount_Hierarchy__c":0.0,"rC_Giving__Expected_Giving_Amount__c":0.0,"rC_Giving__External_ID__c":null,"rC_Giving__GAU__c":null,"rC_Giving__Giving_Type_Engine__c":null,"rC_Giving__Giving_Type__c":null,"rC_Giving__Is_Parent__c":false,"rC_Giving__Is_Shopper__c":false,"rC_Giving__Net_Revenue_Hierarchy__c":0.0,"rC_Giving__Net_Revenue__c":0.0,"rC_Giving__Omitted_Amount_Hierarchy__c":0.0,"rC_Giving__Omitted_Amount__c":0.0,"rC_Giving__Omitted_Bar__c":"<img src=\"/img/samples/color_red.gif\" alt=\"red\" height=\"10\" width=\"0\" border=\"0\"/> 0%","rC_Giving__Omitted_Ratio__c":0.0,"rC_Giving__Percent_Goal_Hierarchy__c":null,"rC_Giving__Percent_Goal__c":0.0,"rC_Giving__Pipeline_Amount_Hierarchy__c":0.0,"rC_Giving__Pipeline_Amount__c":0.0,"rC_Giving__Pipeline_Bar__c":"<img src=\"/img/samples/color_yellow.gif\" alt=\"yellow\" height=\"10\" width=\"0\" border=\"0\"/> 0%","rC_Giving__Pipeline_Ratio__c":0.0,"rC_Giving__ROI_Hierarchy__c":null,"rC_Giving__ROI__c":null,"rC_Giving__Refunded_Amount_Hierarchy__c":0.0,"rC_Giving__Refunded_Amount__c":0.0,"rC_Giving__Refunded_Bar__c":"<img src=\"/img/samples/color_red.gif\" alt=\"red\" height=\"10\" width=\"0\" border=\"0\"/> 0%","rC_Giving__Refunded_Ratio__c":0.0,"rC_Giving__Remaining_Goal_Hierarchy__c":null,"rC_Giving__Remaining_Goal__c":0.0,"rC_Giving__Response_Mechanism__c":null,"rC_Giving__Response_Rate_Hierarchy__c":null,"rC_Giving__Response_Rate__c":0.0,"rC_Giving__Rollup_Giving__c":"2014-05-28T17:53:55.000+0000","rC_Giving__Segment__c":null,"rC_Giving__Send_To_Email_Campaign__c":false,"rC_Giving__Solicitation_List__c":null,"rC_Giving__Solicitation_Type__c":null,"rC_Giving__Source_Code_13_Digits__c":null,"rC_Giving__Source_Code_Partial__c":null,"rC_Giving__Source_Code__c":null,"rC_Giving__Start_Date_Time__c":null,"rC_Giving__Sub_Affiliation__c":null,"rC_Giving__Sub_Channel__c":null,"rC_Giving__Support_Designation__c":null,"rC_Volunteers__Assigned_Volunteer_Count__c":0.0,"rC_Volunteers__Assigned_Volunteer_Hours__c":0.0,"rC_Volunteers__Completed_Volunteer_Count__c":0.0,"rC_Volunteers__Completed_Volunteer_Hours__c":0.0,"rC_Volunteers__Groups__c":false,"rC_Volunteers__Job_Description__c":null,"rC_Volunteers__Match_Age_Range__c":null,"rC_Volunteers__Match_Cause_Areas__c":null,"rC_Volunteers__Match_Gender__c":null,"rC_Volunteers__Match_Role__c":null,"rC_Volunteers__Match_Skills__c":null,"rC_Volunteers__Open_Volunteer_Count__c":0.0,"rC_Volunteers__Open_Volunteer_Hours__c":0.0,"rC_Volunteers__Project_Description__c":null,"rC_Volunteers__Project_End_Date__c":null,"rC_Volunteers__Project_Start_Date__c":null,"rC_Volunteers__Public__c":false,"rC_Volunteers__Required_Volunteer_Count__c":0.0,"rC_Volunteers__Required_Volunteer_Hours__c":0.0,"rC_Volunteers__Rollup_Campaigns__c":null,"rC_Volunteers__Rollup_Members__c":null,"rC_Volunteers__Shift_Date__c":null,"rC_Volunteers__Shift_End_Time__c":null,"rC_Volunteers__Shift_Repeats_Day_Of_Week__c":null,"rC_Volunteers__Shift_Repeats_End_Date__c":null,"rC_Volunteers__Shift_Repeats_Frequency__c":null,"rC_Volunteers__Shift_Repeats_Start_Date__c":null,"rC_Volunteers__Shift_Repeats__c":false,"rC_Volunteers__Shift_Start_Time__c":null,"External_ID__c":null,"rC_Event__Added_To_Waitlist_Text__c":null,"rC_Event__Available_Registered_Count__c":null,"rC_Event__Available_Waitlisted_Count__c":null,"rC_Event__End_Date_Time__c":null,"rC_Event__Event_Code__c":null,"rC_Event__Event_Subtype__c":null,"rC_Event__Event_Type__c":null,"rC_Event__Hidden_Tab_Attendance__c":false,"rC_Event__Hidden_Tab_Attributes__c":false,"rC_Event__Hidden_Tab_Chatter__c":false,"rC_Event__Hidden_Tab_Details__c":false,"rC_Event__Hidden_Tab_Form__c":false,"rC_Event__Hidden_Tab_Groups__c":false,"rC_Event__Hidden_Tab_Meals__c":false,"rC_Event__Hidden_Tab_Members__c":false,"rC_Event__Hidden_Tab_Seating__c":false,"rC_Event__Hidden_Tab_Sessions__c":false,"rC_Event__Hidden_Tab_Tasks__c":false,"rC_Event__Hidden_Tab_Tickets__c":false,"rC_Event__Hidden_Tab_Venues__c":false,"rC_Event__Parent_Name__c":"Unsure","rC_Event__Pricebook_Link__c":"Not assigned [<a href=\"/apex/rc_events__campaign_managepricebooks?id=701G0000000tONi\" target=\"_self\">Change</a>]","rC_Event__Pricebook_Name__c":null,"rC_Event__Pricebook__c":null,"rC_Event__Primary_Venue__c":null,"rC_Event__Registered_Count__c":null,"rC_Event__Registered_Full_Date__c":null,"rC_Event__Registered_Full_Text__c":null,"rC_Event__Registered_Limit__c":null,"rC_Event__Registered_Status__c":null,"rC_Event__Registration_Closed_Message__c":null,"rC_Event__Registration_Edit_Deadline__c":null,"rC_Event__Registration_End_Date_Time__c":null,"rC_Event__Registration_Landing_Page__c":null,"rC_Event__Registration_Start_Date_Time__c":null,"rC_Event__Rollup_Members__c":null,"rC_Event__Rollup_Venues__c":null,"rC_Event__Session_Status__c":null,"rC_Event__Special_Instructions__c":null,"rC_Event__Start_Date_Time__c":null,"rC_Event__Time_Zone__c":null,"rC_Event__Venue_Count__c":null,"rC_Event__Waitlisted_Count__c":null,"rC_Event__Waitlisted_Full_Date__c":null,"rC_Event__Waitlisted_Full_Text__c":null,"rC_Event__Waitlisted_Limit__c":null,"rC_Event__Waitlisted_Status__c":null,"rC_Event__Attended__c":0.0,"rC_Event__No_Show__c":0.0,"Participation__c":null,"Outreach__c":false,"Troop_Formation_Status__c":"N/A","Display_on_Website__c":true,"Grade__c":null,"Program_Grade_Level__c":null,"GS_Community_Year__c":null,"Desired_Girl__c":null,"Girls_Assigned__c":0.0,"Girl_Openings_Remaining__c":0.0,"Account__c":null,"Zip_Code__c":null,"Event_Code__c":null,"Background_Check_Needed__c":false,"Special_Handling__c":false,"Meeting_Location__c":null,"Meeting_Day_s__c":null,"Meeting_Start_Date_time__c":null,"Meeting_End_Date_Time__c":null,"Meeting_Notes__c":null,"Council_Code__c":0.0,"Driving_Role__c":false,"Finance_Role__c":false,"GS_Volunteers_Required__c":null,"Job_Code_Category__c":null,"Job_Code_Sub_Category__c":null,"Job_Code__c":null,"Job_Start_Date__c":null,"ProAlexs-MacBook-Pro:~ akobovich$ 
*/
