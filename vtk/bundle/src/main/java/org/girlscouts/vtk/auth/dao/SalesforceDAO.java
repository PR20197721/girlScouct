...
public class SalesforceDAO {
    ...
    public SalesforceDAO(ConnectionFactory connectionFactory) {
        this.connectionFactory = connectionFactory;
        this.errors = new java.util.ArrayList<VtkError>();
    }

    public SalesforceDAO(ConnectionFactory connectionFactory, SessionFactory sessionFactory) {
    ...
    public User getUser(ApiConfig apiConfig) throws IllegalAccessException {
        return getUserByUserId(apiConfig, apiConfig.getUserId());
    }

    public User getUserByUserId(ApiConfig apiConfig, String userId) throws IllegalAccessException {
        this.errors = new java.util.ArrayList<VtkError>();
        User user = new User();
        ...
        try {
            ...
            if (!apiConfig.isDemoUser()) {
                ...
                log.debug("user info response >>>>> " + rsp);
            } else {
    ...
    public java.util.List<Contact> getContacts(ApiConfig apiConfig,
        ...
        try {
            if (!apiConfig.isDemoUser()) {
                ...
                log.debug("contacts info response >>>>> " + rsp);
                rsp = "{\"records\":" + rsp + "}";
    ...
    public java.util.List<Troop> troopInfo(User user, ApiConfig apiConfig,
        ...
        try {
            ...
            if (!apiConfig.isDemoUser()) {
                ...
                entity.getContent();
                rsp = EntityUtils.toString(entity);
                log.debug("troop info response >>>>> " + rsp);
                if (apiConfig.isUseAsDemo())
...
//if( i==0){ troop.setGradeLevel("7-Multilevel"); }
    ...
    public java.util.List<Contact> getTroopLeaderInfo(ApiConfig apiConfig,
        ...
        try {
            ...
            log.debug("troop leader info response >>>>> " + rsp);
            rsp = "{\"records\":" + rsp + "}";
            try {
