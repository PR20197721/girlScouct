                  ...
                  System.err.println("CHECK MASTER 2");             
                    	...
                    	try{
                    		...
                    		if( email !=null &&
                    			   ...
                    			   log.debug("CHECK MASTER 4: USER2: "+ config.getAccessToken() +" : "+ get.getStatusCode() + " : " + get.getResponseBodyAsString());   
...
System.err.println("tata start "+ sfTroopId);	
	...
	NameValuePair[] params = new NameValuePair[1];
	params[0] = new NameValuePair("q", "select id, email, phone, name, secondary_role__c from Contact where id in (select contactid from campaignmember where campaignid='"+ sfTroopId +"')");
