...
public class SalesforceDAO {
	...
	public User getUser(ApiConfig config) {
		...
		log.debug("CHECK MASTER start1.0 : " + (config == null));
		log.debug("CHECK MASTER start1.1 : " + config.getInstanceUrl());
		...
		log.debug("CHECK MASTER start 3");
		// set the SOQL as a query param
		NameValuePair[] params = new NameValuePair[1];
		log.debug("CHECK MASTER start 4");
		/*
		...
		params[0] = new NameValuePair(
				...
				"SELECT ID,name,email, phone, mobilephone, ContactId, FirstName  from User where id='"
						+ config.getUserId() + "' limit 1");
		log.debug("CHECK MASTER start 5");

		get.setQueryString(params);
		log.debug("CHECK MASTER start 6");
		try {
			log.debug("CHECK MASTER SQL");
			...
			log.debug("CHECK MASTER log USER: " + config.getAccessToken()
					+ " : " + get.getStatusCode() + " : "
					+ get.getResponseBodyAsString());

			log.debug(get.getStatusCode() + " : "
					+ get.getResponseBodyAsString());
			log.debug("CHECK MASTER 0: " + get.getStatusCode() + " : "
					+ get.getResponseBodyAsString());
			if (get.getStatusCode() == HttpStatus.SC_OK) {
				try {
					...
					try {
						...
						log.debug("CHECK MASTER 2");

						try {
							String email = results.getJSONObject(current)
									.getString("Email");
							log.debug("CHECK MASTER 3: " + email);
	...
	public ApiConfig doAuth(String code) {
		...
		try {
			...
			log.debug("CHECK MASTER doAuth: " + post.getResponseBodyAsString());
			if (post.getStatusCode() == HttpStatus.SC_OK) {
				try {
					...
					log.debug("CHECK MASTER doAuth1 "
	...
	public java.util.List<Troop> troopInfo(ApiConfig apiConfig, String contactId) {
		...
		try {
			...
			log.debug("Master token: " + ubConf.getMasterSalesForceToken());
			...
			log.debug(" ctest : SELECT parentid,parent.name,parent.program_grade_level__c, parent.council_code__c, parent.account__c FROM campaign "
					+ "WHERE id IN (SELECT campaignid from campaignmember where  contactid='"
					+ contactId
					+ "' and active__c = true)  "
					+ "AND job_code__c = 'DP' and"
				...
				if (get.getStatusCode() == HttpStatus.SC_OK) {
					...
					try {
						...
						for (int i = 0; i < results.length(); i++) {
							...
							try {
								/*
								 * troop.setCouncilCode(
								 * results.getJSONObject(i)
								 * .getJSONObject("Parent"
								 * ).getInt("Council_Code__c") ); //girls id 111
								 * troop.setCouncilId(results.getJSONObject(i).
								 * getJSONObject
								 * ("Parent").getString("Account__c") );
								 * 
								 * troop.setGradeLevel(results.getJSONObject(i).
								 * getJSONObject
								 * ("Parent").getString("Program_Grade_Level__c"
								 * ) );
								 * troop.setTroopId(results.getJSONObject(i)
								 * .getString("ParentId")); troop.setTroopName(
								 * results
								 * .getJSONObject(i).getJSONObject("Parent"
								 * ).getString("Name") );
								 */
					...
					try {
						...
						for (int i = 0; i < results.length(); i++) {
							...
							try {
								...
								// troop.setCouncilCode(
								// results.getJSONObject(i).getJSONObject("Owner").getInt("Council_Code__c")
								// ); //girls id 111
