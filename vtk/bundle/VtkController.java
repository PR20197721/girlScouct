...
public class VtkController {
    ...
    public VtkController(User user, Troop troop) {
        this.user = user;
        this.troop = troop;
    }
    ...
    public void route(javax.servlet.http.HttpServletRequest request,
        ...
        try {
            ...
            <%
			    ...
			    %><option value = "<%=userTroop.getHash()%>" < %= troop.getHash().equals(userTroop.getHash()) ? "selected" : "" % >> < %= troops.get(i).getTroopName() % > : <%=
                    userTroop.getGradeLevel() % ></option ><%
                }
		   %>
            </select ><%
	      ...
	      <div class="row" >
	       ...
	       <
                    ...
                    onclick = "chgYearPlan('<%=yearPlan.getId()%>', '<%=yearPlan.getPath()%>', '<%=confMsg%>', '<%=yearPlan.getName()%>', <%=troop.getYearPlan()!=null ? true: false %> ,'<%=troop.getYearPlan()!=null ? troop.getYearPlan().getName() : "
                    " %>' )" / >
	        ...
	        <div class="small-18 columns large-pull-2 medium-pull-2 small-pull-2" >
	            <a href = "#"
                    onclick = "chgYearPlan('<%=yearPlan.getId()%>', '<%=yearPlan.getPath()%>', '<%=confMsg%>', '<%=yearPlan.getName()%>')" ><%=
                    yearPlan.getName() % ></a >
	      ...
	      <%}%>
	        ...
	        <div class="row" >
	            ...
	            <%if (condition) { %>
		            ...
		            <
                    input type = "radio" < %= (troop.getYearPlan() != null && (troop.getYearPlan().getName().equals("Custom Year Plan"))) ? " checked " : "" % > id = "r_0"
                    class="radio1" name = "group1" onclick = "chgCustYearPlan('<%=troop.getYearPlan()==null ? "
                    " : troop.getYearPlan().getId()%>', '<%=troop.getYearPlan()==null ? "
                    " :troop.getYearPlan().getPath()%>', '<%=confMsg%>', '<%=troop.getYearPlan()==null ? "
                    " :troop.getYearPlan().getName()%>')" / >
		            <label for="r_0" ></label > </div >
	            <%} %>
	       

	        <div class="small-18 columns large-pull-2 medium-pull-2 small-pull-2" style = "<%= condition ? "
                padding - left:16 px " : " "  %>" >
	        	<div style = "margin-left:-10px;margin-right: -10px;" >
	            <a onclick = "return chgCustYearPlan('<%=troop.getYearPlan()==null ? "
                " : troop.getYearPlan().getId()%>', '<%=troop.getYearPlan()==null ? "
                " : troop.getYearPlan().getPath()%>', '<%=confMsg%>', '<%=troop.getYearPlan()==null ? "
                " : troop.getYearPlan().getName()%>')" >
	            ...
	            <%if (troop != null && troop.getSfTroopAge() != null &&
                 ...
                 <%} else if (troop != null && troop.getSfTroopAge() != null &&
                      ...
                      <h4 style = "color:#18aa51;margin-bottom:15px !important;" > Create Your Multi -Level Troop Year
                    Plan</h4 >
                 <%} else { %>
	            ...
	            <p >
                    ...
                    sb.append("<p style=\"margin-bottom:15px !important;\">");
                    sb.append("All Girls Scouts plan have been organized so you can easily filter through the set to select the right ones for your multi-level troop.. Once your meeting selections are made you'll be able to arrange and finalize the dates in the Year Plan view." +
                            "</p>" +
	      ...
	      <%
                ...
                newTroopCloned.setPermissionTokens(permis);
                troop.setTroop(newTroopCloned);
                if (!troopDAO.isArchivedYearPlan(user, troop, "" + VtkUtil.getCurrentGSYear())) {
                    troop.setYearPlan(null);
                }
                ...
                out.println("<script>self.location='/content/girlscouts-vtk/en/vtk.html';</script>");
