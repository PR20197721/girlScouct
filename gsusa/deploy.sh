...
for server in ${SERVER_LIST[@]}; do
	...
	for port in ${PORT_LIST[@]}; do
		...
		if [ $? -ne 0 ]; then
			...
			curl -u admin:admin -F file=@"$HOME/.m2/repository/org/girlscouts/web/gsusa/$VERSION/gsusa-content-$VERSION.zip" -F name="girlscouts-app" -F force=true -F install=true http://$server:$port/crx/packmgr/service.jsp
