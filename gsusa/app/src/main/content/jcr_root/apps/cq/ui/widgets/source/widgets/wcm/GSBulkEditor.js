 ...
 */
CQ.wcm.GSBulkEditor = CQ.Ext.extend(CQ.Ext.Panel, {
...
"colsMetadata": {
    ...
    * Returns the metadata of the given column id, {} if not found
    ...
    getColObject: function(recreate) {
        if( recreate || !this.currentColObject) {
            ...
            var isDeepMode = this.getIsDeepMode();
    ...
    * Loads the grid.
    ...
    //Girl Scouts - gives user option to set deep search
    getIsDeepConfig: function(){
    	var config = {
    	...
    	if(this.initialConfig && this.initialConfig.isDeepInput){
    		config = CQ.Util.applyDefaults(this.initialConfig.isDeepInput,config);
    	}
    	return config;
    },
    ...
    getIsDeepMode: function() {
    	if(this.isDeepModeInput && this.isDeepModeInput.rendered){
    		return this.isDeepModeInput.getValue();
    	}
