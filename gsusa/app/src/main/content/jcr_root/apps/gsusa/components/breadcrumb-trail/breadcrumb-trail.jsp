...
<%@page import="com.day.cq.wcm.api.WCMMode, org.apache.sling.api.resource.Resource" %>
<%!
    int MAX_TITLE = 58;
...
<ul class="breadcrumb inline-list"><%
    ...
    while (level < currentLevel - 1) {
        ...
        Resource trailRcs = trail.adaptTo(Resource.class);
        trailRcs = trailRcs.getChild("jcr:content");
        if (!trailRcs.isResourceType("girlscouts/components/placeholder-page") && !trailRcs.isResourceType("gsusa/components/placeholder-page")) {
            if (trail == null) {
                break;
            }
            ...
            %>
            <li><%= xssAPI.filterHTML(delim) %><a href="<%= xssAPI.getValidHref(trail.getPath()+".html") %>"><%= xssAPI.encodeForHTML(title) %>
            </a></li>
            <%
             delim = delimStr;
