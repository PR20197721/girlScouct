...
//                    console.log(e);
    ...
    getDeleteConfig: function() {
        ...
        var allowed = user.hasPermissionOn("delete", path);
        /*if (!user.hasPermissionOn("replicate", path)) {
            try {
                // if page is not published, allow deletion
                var pageInfo = CQ.WCM.getPageInfo(path);
                allowed = pageInfo.status.replication.action !== "ACTIVATE";
            } catch (e) {
                // ingnore
            }
        }*/
        return {
            "text": allowed ? this.deleteText : "Delete Page (approvers only)",
            "disabled": !allowed,
            "handler": function() {
                var sidekick = CQ.wcm.Sidekick.findSidekick(this);
                var label = path.substring(path.lastIndexOf("/") + 1);
        ...
        var callback = function(options, success, response) {
            ...
            if (status == 200) {
                ...
                "title":CQ.I18n.getMessage("Delete Page"),
                    "msg":CQ.I18n.getMessage("This page is referenced. Click 'yes' to proceed deleting the page, click 'no' to review the references or 'cancel' to cancel the operation."),
    ...
    getPublishConfig: function(currentPage) {
        ...
        var allowed = CQ.User.getCurrentUser().hasPermissionOn("replicate", this.getPath());
        return {
            "text": allowed ? this.publishText : "Activate Page (approvers only)",
            "disabled": !allowed,
    ...
    getDeactivateConfig: function(currentPage) {
        ...
        var allowed = CQ.User.getCurrentUser().hasPermissionOn("replicate", this.getPath());
        return {
            text: allowed ? this.deactivateText : "Deactivate Page (approvers only)",
            disabled: !allowed,
            hidden: currentPage.isLaunch(),
                        ...
                        "activate": function() {
                            ...
                            for (var i = 0; i < r.records.length; i++) {
                                ...
                                revisionsTable[data.id] = [
                                    ...
                                    data.title,
                                    data.comment,
                                    CQ.wcm.SiteAdmin.formatDate(new Date(r.records[i].json.createdInMillis)),
                                    data.label,
                                    data.id,
                                    data.deleted,
                                    data.isBaseVersion
                    ...
                    "items": [
                        new CQ.Ext.Button({
                            ...
                            "handler": function() {
                                ...
                                var pageInfo = CQ.WCM.getPageInfo(pagePath,true);
                                 var isInWorkflow = pageInfo.workflow && pageInfo.workflow.isRunning;
                                
                                if (isInWorkflow) {
                                    // stop here as we dont want to start a second workflow on that page
                                    CQ.Ext.Msg.alert(CQ.I18n.getMessage("WARN"), CQ.I18n.getMessage("Page state as changed, workflow cannot be started at this time. Please refresh the page."));
                                    return;
                                }
                ...
                if (languages){
                    for (var i = 0; i<languages.length; i++){
                        ...
                        if(row['path'] != this.path){
                            ...
                            if(row['exists']){
                                cls += "color:black;";
                                img = row['country'] ? ("<a href='" + CQ.shared.HTTP.getXhrHookedURL(CQ.HTTP.externalize(row['path']) + ".html") + "' target='_new' style=\'"+cls+"\' title='"+CQ.I18n.getMessage(row['language'])+"'><img style=\'vertical-align:text-bottom;width:20px;margin-left:5px;\' src=\'" + CQ.shared.HTTP.getXhrHookedURL(flagRoot + row['country'] + ".gif") + "\'></a>") : "";
                            }
                            else{
                                cls += "color:#888888;";
                                img = row['country'] ? ("<a href='" + CQ.shared.HTTP.getXhrHookedURL(CQ.HTTP.externalize(row['path']) + ".html") + "' target='_new' style=\'"+cls+"\' title='"+CQ.I18n.getMessage(row['language'])+"'><img style=\'vertical-align:text-bottom;opacity:0.4;filter:alpha(opacity=40);width:20px;margin-left:5px\' src=\'" + CQ.shared.HTTP.getXhrHookedURL(flagRoot + row['country'] + ".gif") + "\'></a>") : "";
                            }
            ...
            this.previewSwitch.on("click", function() {
                ...
                if (this.collapsed && (!CQ.WCM.isEditMode() || scaffoldMode)) {
                    ...
                    if (scaffoldMode) {
                        ...
                        CQ.Util.reload(CQ.WCM.getContentWindow());
                    } else if (!this.pageLocked || (this.pageLocked && !currentPage.isReadOnly())) {
