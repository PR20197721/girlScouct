...
for server in ${SERVER_LIST[@]}; do
	...
	for port in ${PORT_LIST[@]}; do
		...
		if [ $? -ne 0 ]; then
			...
			curl -u admin:admin -F file=@"$HOME/.m2/repository/org/girlscouts/gsactivities/gsactivities-app/$VERSION/gsactivities-app-$VERSION.zip" -F name="gs-activities" -F force=true -F install=true http://$server:$port/crx/packmgr/service.jsp
