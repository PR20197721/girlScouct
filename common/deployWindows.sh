...
for server in ${SERVER_LIST[@]}; do
	...
	for port in ${PORT_LIST[@]}; do
		...
		if [ $? -ne 0 ]; then
			...
			curl -u admin:admin -F file=@"$HOME/.m2/repository/org/girlscouts/web/girlscouts-common-app/$VERSION/girlscouts-common-app-$VERSION.zip" -F name="girlscouts-common-app" -F force=true -F install=true http://$server:$port/crx/packmgr/service.jsp
...
if [ $? -ne 0 ]; then
    ...
    curl -u admin:admin -F file=@"$HOME/.m2/repository/org/girlscouts/web/girlscouts-common-app/$VERSION/girlscouts-common-app-$VERSION.zip" -F name="girlscouts-common-app" -F force=true -F install=true http://192.168.0.118:4503/crx/packmgr/service.jsp
fi
