...
@Component(service = Servlet.class, property = {
		Constants.SERVICE_DESCRIPTION + "=Girl Scouts Bulk Editor TSV Servlet",
		"sling.servlet.methods=" + HttpConstants.METHOD_GET,
		"sling.servlet.paths=" + "/services/gsbulkeditor/export/tsv" })
public class GirlScoutsBulkEditorTSVServlet extends SlingAllMethodsServlet {
	...
	private static Logger log = LoggerFactory.getLogger(GirlScoutsBulkEditorTSVServlet.class);

	@Activate
	private void activate(ComponentContext context) {
		log.info("Girlscouts GS Bulkeditor TSV Servlet Activated.");
	}
	...
	protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
		...
		log.debug("Girlscouts GS Bulkeditor TSV Servlet processing.");
		...
		try {
			...
			// (request.getParameter(SEPARATOR_PARAM)!=null ?
			// request.getParameter(SEPARATOR_PARAM) : DEFAULT_SEPARATOR);
			final String separator = DEFAULT_SEPARATOR;

			bw.write(valueParser(JcrConstants.JCR_PATH, separator));
			if (properties != null) {
				for (String property : properties) {
					property = property.trim();
					bw.write(separator + valueParser(property, separator));
				}
			}
			...
			while (hits.hasNext()) {
				...
				if (node != null) {
					bw.write(valueParser(hit.getValue(JcrConstants.JCR_PATH).getString(), separator));
					if (properties != null) {
						for (String property : properties) {
							bw.write(separator);
							property = property.trim();
