...
@SlingServlet(
		label = "Girl Scouts Pages References Report Servlet", description = "Generates a csv file report of all pages where page is being referenced", paths = {},
        methods = { "GET", "POST" }, // Ignored if paths is set - Defaults to GET if not specified
		resourceTypes = { "gsusa/servlets/pagereferences" }, // Ignored if paths
																// is set
		selectors = {}, // Ignored if paths is set
        extensions = { "html", "htm" }  // Ignored if paths is set
)
public class PagesReferencesReportServlet extends SlingAllMethodsServlet implements OptingServlet {
	private static final Logger log = LoggerFactory.getLogger(PagesReferencesReportServlet.class);
			...
			try {
				...
				for (String path : paths) {
					...
					for (String page : pagePaths) {
						...
						row.add(page);
						Collection<ReferenceSearch.Info> resultSet = referenceSearch.search(resolver, page).values();
						for (ReferenceSearch.Info info : resultSet) {
							for (String p : info.getProperties()) {
								Resource resource = resolver.resolve(p);
								Resource referrerPage = getPage(resource);
								if (referrerPage != null) {
									if (isActive(referrerPage)) {
										p = referrerPage.getPath() + " (Active)";
									} else {
										p = referrerPage.getPath() + " (Not Active)";
									}
								}
								councils.add(p);
