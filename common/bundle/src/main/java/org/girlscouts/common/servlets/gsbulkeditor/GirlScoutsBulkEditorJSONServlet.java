...
@Component(service = Servlet.class, property = {
        Constants.SERVICE_DESCRIPTION + "=Girl Scouts Bulk Editor JSON Servlet",
        "sling.servlet.methods=" + HttpConstants.METHOD_GET, "sling.servlet.paths=" + "/services/gsbulkeditor/query" })
public class GirlScoutsBulkEditorJSONServlet extends SlingAllMethodsServlet {
    ...
    private static final String DEEP_SEARCH_PARAM = "isDeep";
    private static final String RESOURCE_TYPE_PARAM = "resourceType";
    private static final String PRIMARY_TYPE_PARAM = "primaryType";
    ...
    private static Logger log = LoggerFactory.getLogger(GirlScoutsBulkEditorJSONServlet.class);


    @Activate
    private void activate(ComponentContext context) {
        log.info("Girlscouts GS Bulkeditor JSON Servlet Activated.");
    }

    protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
            throws ServletException, IOException {
        log.debug("Girlscouts GS Bulkeditor JSON Servlet processing.");
        ...
        request.getParameter(COMMON_PATH_PREFIX_PARAM);

        Session session = request.getResourceResolver().adaptTo(Session.class);
        log.debug("No issues on startup");
    ...
    public void writeProperty(TidyJSONWriter writer, String name, Property prop, ResourceResolver rr) throws Exception {
        writer.key(encodeString(name));
        log.debug("Writing property " + name);
    ...
    public void iterateNodes(String path, long nbrOfResults, TidyJSONWriter writer, String[] properties,
                             Session session, SlingHttpServletRequest request, Boolean isDeep, String resourceType, String primaryType,
                             String importType) throws Exception {
        NodeIterator iter = session.getNode(path).getNodes();
        log.debug("Iterating from node " + path);
        while (iter.hasNext()) {
            // Girl Scouts customization
            // Use NodeIterator instead. Only handles direct children.
            // This reduces deeper query and gets rid of the memory issue of
            // queries.
            // while (hits.hasNext()) {
                ...
                if (canIterate) {
                    if (node.hasNodes() && isDeep) {
                        iterateNodes(node.getPath(), nbrOfResults, writer, properties, session, request, isDeep,
                                resourceType, primaryType, importType);
                    }
                    continue;
                }
                ...
                if (node.hasProperty("isEncrypted") && node.getProperty("isEncrypted").getString().equals("true")) {
                    ...
                    if (properties != null) {
                        for (String property : properties) {
                            if (null != importType) {
                                if (importType.equals("events")) {
                                    if (property.equals("jcr:content/data/start-date")
                                        ...
                                        if (node.hasProperty(property.substring(0, property.lastIndexOf("-")))) {
                                            String datetimeString = node
                                                    .getProperty(property.substring(0, property.lastIndexOf("-")))
                                                    .getString();
                                            GSDateTimeFormatter dtfIn = GSDateTimeFormat
                                            ...
                                            } else if (property.equals("jcr:content/data/start-time")
                                                    || property.equals("jcr:content/data/end-time")
                                                    || property.equals("jcr:content/data/regOpen-time")
                                                    || property.equals("jcr:content/data/regClose-time")) {
                                    ...
                                    } else if (property.equals("jcr:content/cq:tags-progLevel")
