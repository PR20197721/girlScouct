...
;(function ($, ns, channel, window, undefined) {
    ...
    ns.MsmAuthoringHelper = ns.MsmAuthoringHelper || (function() {
        ...
        var _initPageInfo = function(callback) {
            ...
            $.ajax({
                ...
                console.log(data);
				console.log(MSM.MSMCommons.Constants.PROP_SOURCE_PATH+"="+data[MSM.MSMCommons.Constants.PROP_SOURCE_PATH]);
                console.log(MSM.MSMCommons.Constants.PROP_TARGET_PATH+"="+data[MSM.MSMCommons.Constants.PROP_TARGET_PATH]);
                if (callback) {
                    callback.call(this);
                }
            });
        ...
        var _updateDropTargetState = function(editable) {
            if (MSM.MSMCommons.isInheritanceCancelled(editable)
                ...
                if (editable.dropTargets.length > 0) {
    ...
    $("#OverlayWrapper").on("click", ".msm-rollout-option", function(e) {
        ...
        if (el.find("[data-dialogpath]").length > 0) {
            ...
            if (rolloutCommandPath
                    && blueprint) {
                MSM.Rollout.doRollout(Granite.HTTP.externalize(rolloutCommandPath), blueprint);
            }
        }
    });
    ...
    channel.on("cq-persistence-after-move", function(event, editable) {
        ...
        if (!ns.MsmAuthoringHelper.isInheritanceCancelled(parent)) {
